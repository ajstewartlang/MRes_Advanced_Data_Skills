...
To being with, you need to select a new R Script under the File -> New File menu option (see below). You're going to write all of the following R coded in that script window.
<br>
<br>
![](image3.png) 
<br>
...
It looks like some participants appear more than once in the dataset - the first few rows are all for participant ID 51624 - so looks like there is duplicate data. We can use the `unique()` function alongwith the `length()` function to tell us the unique number of IDs in the dataset. We use the symbol $ to refer to a pariciular column in our data.  So, `NHANES$ID` refers to the column "ID" in the dataset NHANES.
...
So it looks like those with College eduction have the lowest median BMI (ignoring the NA category which corresponds cases where we don't have Eduction level recorded).  
...
NHANES_tidied %>% 
  ...
  ggplot(aes(x = fct_reorder(Education:Work, BMI, median), y = BMI, 
...
NHANES_tidied %>% 
  filter(!is.na(Education) & !is.na(Work) & !is.na(BMI)) %>%
  ggplot(aes(x = fct_reorder(Work, BMI, median), y = BMI, 
              colour = Work)) + 
  geom_boxplot() + 
  coord_flip() +
  guides(colour = FALSE) + 
...
NHANES_tidied %>% 
  filter(!is.na(Education) & !is.na(Work) & !is.na(BMI)) %>%
  ggplot(aes(x = fct_reorder(Education, BMI, median), y = BMI, 
              colour = Education)) + 
  geom_boxplot() + 
  coord_flip() +
  guides(colour = FALSE) + 
...
NHANES_tidied %>% 
  ...
  ggplot(aes(x = fct_reorder(Race1, BMI, median), y = BMI, colour = Race1)) +
  geom_boxplot() + 
  guides(colour = FALSE) +
