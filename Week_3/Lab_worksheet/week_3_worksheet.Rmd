...
It looks like some participants appear more than once in the dataset - the first few rows are all for participant ID 51624 - so looks like there is duplicate data. We can use the `select()` function alongwith the `distinct()` and `tally()` functions to tell us the unique number of IDs in the dataset. 
